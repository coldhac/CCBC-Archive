---
record_id: "ccbc13-14:ccbc-13-13"
event_id: "ccbc13-14"
editions: ["CCBC 13"]
year: 2023
area: "CCBC-13"
kind: "puzzle"
source_url: "https://archive.cipherpuzzles.com/ccbc13/problems/CCBC-13/13.yaml"
---
...
export default {
  setup() {
    ...
    word_set = null;
    const init = async () => {
      let dataResponse = await fetch('../../../assets/archive.cipherpuzzles.com/ccbc13/problems/CCBC-13/scowl.json')
    ...
    function MYOPHSpFuncInternal(input, skip) {
        ...
        contents.push("<li>" + sbase3 + "你决定小题答案的单词不应该过于生僻。例如 <code>CURRENT GHOST BAMBOO</code> 是一个合法的答案，而 <code>ORME SHOE</code> 则不是。你有一个<a href=\"../../../assets/static.cipherpuzzles.com/static/images/b08086b053de4903a46d7840b7691516.txt\">字典</a>，其中包含了所有你认为合法的单词。</li>");
        // meta_num
...
按照要求构造即可。构造时可使用 [Nutrimatic](https://nutrimatic.org/)、 [Qat](https://www.quinapalus.com/cgi-bin/qat) 或 [onelook](https://www.onelook.com/) 等搜词网站，也可以写代码（因为词典文件已给出）。
...
* 第四部分（在满足第三部分之后出现）：会得到一个长度26的包含大写字母和问号的字符串。经过观察或尝试后可以猜到字符串第i位对应第i个大写字母开头的单词，如果单词包含想要的字母即会显示该字母，否则则显示问号。通过修改小题答案以及进行一定的猜测后可以得到 ```FINAL ANSWER IS STAFF SERGEANT```。最终答案即为 ```STAFF SERGEANT```。
...
可使用 [Nutrimatic](https://nutrimatic.org/)、 [Qat](https://www.quinapalus.com/cgi-bin/qat) 或 [onelook](https://www.onelook.com/) 等搜词网站，也可以写代码（因为词典文件已给出）。

词典文件没有特殊性质，正常的单词都在给出的词典里。

### 2. 该如何爆猜（提取）

注意到最终元谜题的要求需要小题答案总共有26个单词（并且这些单词有一个自然的顺序），显示的最终线索长度也恰好是26。对于最终线索的每个位置，如果对应的单词包含想要的字母即会显示该字母，否则则显示问号。


## 本地附件

- [scowl.json](../../../assets/archive.cipherpuzzles.com/ccbc13/problems/CCBC-13/scowl.json)
- [b08086b053de4903a46d7840b7691516.txt](../../../assets/static.cipherpuzzles.com/static/images/b08086b053de4903a46d7840b7691516.txt)

来源：[https://archive.cipherpuzzles.com/ccbc13/problems/CCBC-13/13.yaml](https://archive.cipherpuzzles.com/ccbc13/problems/CCBC-13/13.yaml)
